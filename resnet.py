...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        #if self.dims == 3:
        #    padding = (7, 7, 7, 7, 1, 0) # [?, 1, 79, 98, 82] -> [?, 1, 80, 112, 96]
        #else:
        #    padding = (6,6) # [?, 420] -> [?, 432]
        #print("input: ", x.shape)
        #x = F.pad(x, padding)
...
def resnet_test(dims=3, **kwargs):
    return _resnet('', BasicBlock, [1, 1, 1, 1], False, False, dims=dims, **kwargs)
...
def resnet18(pretrained=False, progress=True, dims=3, num_class=1000, **kwargs):
    ...
    #return _resnet('resnet18', Bottleneck, [2, 2, 2, 2], pretrained, progress, num_class=num_class, dims=dims, **kwargs)
